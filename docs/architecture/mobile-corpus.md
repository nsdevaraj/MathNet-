...
Android checksum verification runs in the app-local `CorpusChecksum` Capacitor plugin on a dedicated worker. It reads through a fixed 64 KB buffer and sends at most 100 progress updates plus the final SHA-256 digest to the WebView. Do not replace this with `Filesystem.readFileInChunks` on Android: that push stream has no consumer backpressure and can queue the entire base64-encoded database behind JavaScript hashing. iOS retains incremental JavaScript verification. The setup screen reports download and verification percentages separately, then shows an indeterminate finalizing state during SQLite validation and activation.

The native checksum tests cover a 214,937,600-byte fixture, bounded progress, missing files, and interruption. Run them with `cd android && ./gradlew testDebugUnitTest --tests com.devois.devapp.CorpusChecksumTest` after configuring JDK 21 and the Android SDK. The checksum helper and tests can also run directly with JUnit 4.13.2 under `java -Xmx32m` to check that corpus size does not determine heap usage. Installer regressions run with `npx tsx --test src/corpus/nativeCorpusInstaller.test.ts`.
